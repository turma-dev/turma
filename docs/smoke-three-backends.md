...
> **Why `git restore` before each `turma run`:** any interleaved `bd` command —
> `plan-to-beads --force` here, and `bd show` during verification — re-exports
> `.beads/issues.jsonl` and leaves it dirty in the working tree, which
