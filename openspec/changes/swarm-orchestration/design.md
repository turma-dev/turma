...
class BeadsAdapter:
    ...
    def fail_task(
        ...
        *,
        ...
        #
        # NOT idempotent across its own internal steps. Each of the
        # three primitives (bd note, label add/remove, status
        # release) is an independent bd subcommand; a failure between
        # them (for example, note lands but label add then fails)
        # leaves partial state that a blind retry would duplicate
        # (second note, same label applied twice, etc.).
        #
        # Partial-failure recovery is the operator's job: inspect
        # `bd show <task_id>` to see what landed, resolve any
        # duplication via direct `bd` commands, and re-run
        # `turma run` only once the task is back in a clean
        # starting state. The adapter surfaces each internal step's
        # argv + bd stderr in the PlanningError so triage is
        # concrete.
        ...
...
- `WorktreeManager.cleanup(path)` removes the worktree and its
  branch. Called only when a task reaches `closed` in Beads — that
  is, on the happy path (`close_task` after a successful PR) or on
  a `completion-pending-with-pr` reconcile repair.

**Failed worktrees are never auto-removed.** Retries-remaining and
budget-exhausted failures BOTH leave the worktree on disk as the
primary triage artifact; reconciliation picks up the `.task_failed`
sentinel on the next run and the operator inspects the worktree
contents to understand what happened. Removal of a failed worktree
is an explicit manual step (`git worktree remove --force
<path>` + `git branch -D task/<feature>/<task_id>`) and a future
`turma run --clean <feature>` flag is listed in "Open items
deferred past v1".
...
`{task_id}` is the full Beads id (e.g. `bd-smoke-1`). The template
does NOT prepend `bd-` — task ids arrive from bd already prefixed.
...
(The `Closes {task_id}` line is a convention for later automation;
nothing in the orchestrator consumes it yet. `{task_id}` is the full
Beads id — no extra `bd-` prefix.)
...
2. For each, classify based on the worktree at
   ...
   | `failure-pending` | `.task_failed` present | pass to `fail_task` with the worker's reason; leave the worktree on disk for triage |
...
- `.worktrees/<feature>/<bd-id>/.task_failed` — worker wrote this;
  contents are the failure reason. Orchestrator calls `fail_task`
  (budget accounting happens there), leaves the worktree in place
  for triage, and continues the loop (or halts on exhaustion).
...
- A `turma status` dashboard that inspects live orchestrator state
  across features.
- A `turma run --clean <feature>` (or equivalent) to bulk-remove
  worktrees and branches for a feature's failed/abandoned tasks.
  v1 leaves failed worktrees on disk indefinitely; operators clean
  them up manually via `git worktree remove --force` + `git branch
  -D`.
