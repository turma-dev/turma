...
- [x] **Determine the argv for each new operation first.** Run
      ...
      `beads-transcription`.
- [x] Add `list_ready_tasks(feature: str) -> tuple[BeadsTaskRef, ...]`
...
- [x] Add `fail_task(task_id, reason, *, retries_so_far,
      ...
      `needs_human_review` + releases (budget exhausted). **Not
      idempotent across its internal steps** (note + label
      add/remove + status release are separate bd subcommands); a
      failure between steps requires operator triage via
      `bd show <task_id>` before the orchestrator is re-run. The
      adapter surfaces each internal step's argv and `bd` stderr in
      the raised `PlanningError` so triage is concrete.
- [x] `close_task` already exists from `beads-transcription`; add a
      unit test documenting that it remains usable by the swarm
      orchestrator on worker success.
- [x] Full unit coverage in `tests/test_swarm_beads_extensions.py`:
...
- [x] New module `src/turma/swarm/worktree.py`.
- [x] `WorktreeManager(repo_root: Path, worktree_root: str)` with
      methods:
      ...
      - `cleanup(path: Path) -> None`: `git worktree remove` +
        ...
        on task success.
- [x] argv pinned: `git worktree list --porcelain` for
...
- [x] Branch collision (existing `task/<feature>/<task_id>` branch
      without a worktree) raises `PlanningError` with the `git`
      stderr preserved.
- [x] `.gitignore`: add `.worktrees/`.
- [x] Unit tests in `tests/test_swarm_worktree.py`: create new,
...
- [x] New module `src/turma/swarm/worker.py` exposing
      `WorkerInvocation`, `WorkerResult`, and the `WorkerBackend`
      Protocol.
- [x] `ClaudeCodeWorker` implementation:
...
- [x] Worker registry: `_BACKENDS: dict[str, Callable[[],
      WorkerBackend]]` keyed by name. v1 registers only
      `"claude-code"`.
- [x] `.gitignore`: add `.task_complete`, `.task_progress`,
      `.task_failed`.
- [x] Unit tests in `tests/test_swarm_worker.py`: prompt renders
...
- [x] Non-zero exits on any git call raise `PlanningError` with
...
- [x] Commit-message template lives as a module-level constant that
      the orchestrator renders. Pinned shape (`{task_id}` is the full
      Beads id; do NOT prepend `bd-`):
      ```
      ...
      ```
- [x] Credentials are assumed to already work (ssh agent or
      `gh auth` credential helper). Adapter does not manage creds.
- [x] Unit tests in `tests/test_swarm_git.py` with subprocess stubs:
...
- [x] **Read-only.** The module detects ambiguity and returns a
...
- [x] `ReconciliationReport.findings` is an ordered tuple of typed
...
- [x] Unit tests in `tests/test_swarm_reconciliation.py`: each
...
- [x] **Repair phase (between reconciliation and fetch_ready).**
      ...
      - `failure-pending` → fail_task with the worker's reason.
        Leave the worktree in place for operator triage — it is
...
- [x] Budget enforcement: read `retries_so_far(task_id)` before
      ...
      (default unbounded).
- [x] `SwarmServices` dataclass for dependency injection (Beads +
      Worktree + Git + PR adapters + worker factory), mirroring
      `PlanningServices` and the transcription shape. Tests inject
      stubs directly.
- [x] Integration tests in `tests/test_swarm_run.py` using
...
- [x] Replace the placeholder `turma run` in `src/turma/cli.py` with
      the real dispatch: `--feature` required; `--max-tasks`,
      `--backend`, `--dry-run` optional.
- [x] Construct the default services (real `BeadsAdapter`,
      `WorktreeManager`, `GitAdapter`, `PullRequestAdapter`,
      registered worker backend) and call `run_swarm`.
- [x] Map `PlanningError` to exit 1 with `error: <message>` on
      stdout, matching the pattern in `turma plan` and
      `turma plan-to-beads`.
- [x] Print a compact per-task summary as the loop progresses
      (claim → PR URL → close), mirroring the resume / transcription
      output style.
- [x] CLI tests in `tests/test_swarm_cli.py`: subparser registered,
...
- [x] Update `turma.example.toml` with the documented `[swarm]`
      block: `worker_backend`, `worker_timeout`, `max_retries`,
      `worktree_root`, `base_branch` (all with v1 defaults).
- [x] Update `README.md` with a new "Swarm Execution" section:
...
- [x] Update `docs/architecture.md` Execution section from its
      current high-level framing to the committed v1 contract.
- [x] New `docs/smoke-turma-run.md` runbook — same shape as
...
- [x] `CHANGELOG.md` `[Unreleased]` entry rolling up the change
      set.
- [x] No new runtime deps in `pyproject.toml`. `gh` and `claude` are
