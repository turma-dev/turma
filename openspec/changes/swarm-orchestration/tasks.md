...
### 1. Extend BeadsAdapter with ready / claim / fail

- [ ] **Determine the argv for each new operation first.** Run
      `bd --help`, `bd list --help`, and whatever bd subcommands
      handle state transitions (candidates: `bd ready`, `bd start`,
      `bd set-state`, `bd assign`), then pick the minimal argv that
      yields the needed semantics for each adapter method. Document
      the chosen commands in the `BeadsAdapter` class docstring with
      the same argv-pinning test pattern used in Task 2 of
      `beads-transcription`.
- [ ] Add `list_ready_tasks(feature: str) -> tuple[BeadsTaskRef, ...]`
      returning OPEN + `feature:<name>` + unblocked tasks.
- [ ] Add `claim_task(task_id: str) -> None` that atomically
      transitions the task out of "ready" into an "in progress"-
      equivalent state. Raises `PlanningError` with `bd` stderr if
      the task is already claimed, closed, or still blocked.
- [ ] Add `fail_task(task_id: str, reason: str) -> None` that
      records the failure (via `bd note`, event payload, or label —
      whichever Task 1's verification picks) and reopens the task so
      a later run can retry within budget.
...
- [ ] Full unit coverage in `tests/test_swarm_beads_extensions.py`:
      ...
      rejection, fail-path argv, and missing-CLI inherited from the
      existing init check.
...
- [ ] New module `src/turma/swarm/reconciliation.py` with
      ...
      repo_root) -> ReconciliationReport`.
- [ ] Walk the authority order in the design doc:
      Beads "in progress" tasks → worktree presence / sentinels →
      branch + PR state. Produce a typed report naming each
      resolution (closed via PR, re-opened due to missing worktree,
      needs human attention).
- [ ] v1 does NOT auto-close, auto-re-open, or auto-PR during
      reconciliation. It surfaces the report on stdout and leaves
      state mutation to the caller's explicit resume path.
- [ ] Unit tests in `tests/test_swarm_reconciliation.py`: no
      in-progress tasks, missing worktree surfaces as "needs
      attention", `.task_complete` present surfaces as "ready to
      close", stale branch without Beads task surfaces as "orphan
      branch".

### 6. Wire the swarm orchestrator
...
- [ ] Invoke reconciliation (always, even on `--dry-run`). Print
      the report.
- [ ] Main loop: `fetch_ready → claim → setup_worktree →
      run_worker → (sentinel dispatch) → (commit/pr/close) OR
      (fail_task) → next`.
- [ ] Budget enforcement: `max_retries` per task, `max_tasks` across
      the loop (default unbounded).
- [ ] `SwarmServices` dataclass for dependency injection (adapters
      + worktree manager + worker factory), mirroring
      `PlanningServices` and `TranscriptionServices` shape. Tests
      inject stubs directly.
- [ ] Integration tests in `tests/test_swarm_run.py` using
      `StubBeadsAdapter`, `StubWorktreeManager`, `StubWorkerBackend`,
      `StubPullRequestAdapter` from the earlier test files. Cover:
      one-task happy loop, multi-task sequential loop, claim race
      (another run claimed), worker-success path, worker-fail path
      with retry budget, budget exhaustion halts, `--dry-run` is
      read-only, preflight failures.
...
- [ ] Construct the default services (real `BeadsAdapter`,
      `WorktreeManager`, `PullRequestAdapter`, registered worker
      backend) and call `run_swarm`.
...
### 8. Docs, config, and live validation
