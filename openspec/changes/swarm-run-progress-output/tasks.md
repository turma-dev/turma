...
- [ ] In `tests/test_swarm_run.py`, extend the existing happy-path
      ...
      `captured.out`:
  - [ ] `worktree: setup <task id>`
  - [ ] `worker: running <task id> (timeout <worker_timeout>s)` — pinned
        against the stub services' configured `worker_timeout` so the
        value is exact
  - [ ] `commit: <task id>`
  - [ ] `push: <task id>`
- [ ] In the existing worker-failure test, assert `worker: running
...
- [ ] Run the suite and confirm these new assertions fail for the right
      reason (lines absent), before touching `_orchestrator.py`.

### 2. Add the four print lines to `_run_single_task`

- [ ] `print(f"worktree: setup {task.id}")` immediately after
      `services.worktree.setup(...)` returns.
- [ ] `print(f"worker: running {task.id} (timeout
      {services.worker_timeout}s)")` immediately **before**
      `worker.run(invocation)`.
- [ ] `print(f"commit: {task.id}")` inside the `try`, immediately after
      `services.git.commit_all_with_bd_export(...)` returns and before
      `push_branch`.
- [ ] `print(f"push: {task.id}")` immediately after
      `services.git.push_branch(...)` returns.
- [ ] Leave the final `swarm: opened ...` line and all `_handle_failure`
      paths untouched.

### 3. Green + guard the contract

- [ ] Run `tests/test_swarm_run.py`; confirm the new assertions pass and
      no previously-passing `capsys` assertion (including the `not in`
      checks) regressed.
- [ ] Run the full validation baseline: `uv sync`, `uv run turma init`,
      `uv run turma --help`, `uv run python -m turma --help`,
      `uv run pytest`.

### 4. Docs + changelog

- [ ] `docs/architecture.md` Execution section: one sentence noting the
      per-task lifecycle emits `worktree:` / `worker:` / `commit:` /
      `push:` progress lines. State machine diagram unchanged.
- [ ] `CHANGELOG.md` `[Unreleased]`: note the additive `turma run`
      per-task progress output.

### 5. Scope guard

- [ ] Confirm the diff is limited to: four `print` lines in
