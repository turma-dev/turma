...
- [x] **Determine the argv first.** Run `bd list --help` and
      ...
      `list_ready_tasks`.
- [x] Add `list_feature_tasks_all_statuses(feature: str) ->
...
- [x] Unit tests in `tests/test_transcription_beads.py`: argv
...
- [x] **Determine the argv first.** Try
...
- [x] Add a frozen `PrSummary` dataclass
      (`number: int`, `url: str`, `state: str`, `title: str`,
      `head_branch: str`) at module scope.
- [x] Add `list_prs_for_feature(feature: str, worktree_manager:
      ...
      payloads also raise.
- [x] Unit tests in `tests/test_swarm_pull_request.py`: argv
...
- [x] New module `src/turma/swarm/status.py` exposing
...
- [x] Compose the adapter reads in the order pinned by
      `design.md`:
...
- [x] Render per the pinned output shape. Each subsection
...
- [x] Sentinel inspection in the in-progress section reads
...
- [x] New `tests/test_swarm_status.py`. Pattern:
...
- [x] **No-mutation invariant test.** Populate the feature
...
- [x] Task counter correctness across a mixed-status fixture.
- [x] Per-section rendering tests:
      ...
      - Orphan branches section — strict in_progress-only
        ...
        - branch with no corresponding task → rendered as
          orphan.
        - empty case `(none)`.
- [x] Missing spec dir / APPROVED / TRANSCRIBED.md render as
      `no` with the hint line; command still returns a readout
      (does not raise).
- [x] Adapter failure (stubbed `PlanningError` on
...
- [x] Replace the placeholder `turma status` dispatch in
      ...
      commands use).
- [x] Update the argparse subparser to require `--feature`.
- [x] `src/turma/swarm/__init__.py` drops `status_summary` from
      the public re-exports (no remaining callers after the CLI
      rewire).

### 6. CLI tests

- [x] New tests in `tests/test_swarm_cli.py` (or a dedicated
...
- [x] `README.md`: add a "Feature Status" subsection under
...
- [x] `CHANGELOG.md` `[Unreleased]`: single entry under "Added"
      rolling up the new command + the adapter additions.

### 8. Validation

- [x] `uv run pytest` — full suite green. Current baseline
      before this change set: 423 tests. Expected additions are
      ~15 across status + adapter + CLI tests.
- [x] Manual smoke: on the existing `khanhgithead/turma-run-smoke`
      ...
      over them.
- [x] No new runtime dependencies in `pyproject.toml`. `bd` and
