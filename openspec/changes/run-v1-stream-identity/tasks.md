...
> **Implemented 2026-07-22** (branch `feat/run-v1-stream-identity`). Emitter identity + thread-safe writes; `SwarmHalted` + CLI `--json`-gated lifecycle envelope (`run_started`/`run_completed`, outcome from exit path); configurable `[swarm].heartbeat_interval` ticker. 684 passed.
...
- [x] Emitter tests: `JsonEmitter` injects a stable `run_id` + ISO-8601 `ts` into
  ...
  under its lock.
- [x] CLI `run --json` lifecycle tests: `run_started` first (correct mode +
...
- [x] CLI text-mode gating test: `turma run` (no `--json`) emits no `run_started`
  / `run_completed` / `heartbeat` and starts no ticker thread; text byte-for-byte
  assertions unchanged.

### 2. Identity + thread-safety in the emitters
- [x] `JsonEmitter(run_id=None)` — generate a `uuid4` hex if not given; inject
  `run_id` + a fresh `ts` into every payload.
- [x] Wrap `write` + `flush` in a `threading.Lock` (both emitters — the
...
- [x] Add `SwarmHalted(PlanningError)`; `run_swarm` raises it on retry exhaustion
  (replacing the bare `PlanningError` on that path).
- [x] CLI `run` handler, **only when `--json`**: capture a monotonic clock at
...
- [x] `[swarm].heartbeat_interval` config (default 15s, `0` disables) parsed in
  `SwarmConfig`.
- [x] Daemon thread emitting `heartbeat` (`elapsed_ms`) every interval (stop via a
...
### 5. Docs + baseline
- [x] Update `run-json-events` event catalog / `docs/` (`architecture.md` JSON
  surface, any `--json` examples) with `run_id` / `ts` / lifecycle / heartbeat.
- [x] `turma.example.toml`: document `heartbeat_interval`.
- [x] CHANGELOG; full baseline green; scope guard.
