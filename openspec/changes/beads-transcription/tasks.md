...
- [x] New module `src/turma/transcription/tasks_md.py`. Pure; no
      subprocess, no filesystem.
- [x] Parse `## Tasks` + `### <N>\. <title>` section headings + `- [ ]`
      subtask bullets into a typed ordered representation.
- [x] Support optional `[type: impl | test | docs | spec]` inline
      section marker. Unknown token → parse failure.
- [x] Support optional `[blocked-by: N]` / `[blocked-by: N, M]` inline
      section marker. Reject forward reference, self reference, and
      references to non-existent section numbers.
- [x] Default dependency: each section depends on the previous; section
      1 has no edges.
- [x] Default type inference from title keywords (test/tests → test;
      doc/docs/readme → docs; spec/specification → spec; else impl).
- [x] Reject: missing `## Tasks`, non-ascending section numbers, empty
      sections, malformed markers.
- [x] Return a typed ParseResult union: `ParsedTasks(...)` or
      `TasksParseFailure(reason, ...)`.
- [x] Full unit coverage in `tests/test_transcription_tasks_md.py`:
...
- [x] New module `src/turma/transcription/beads.py`.
- [x] `BeadsAdapter.__init__` validates `shutil.which("bd")` and raises
      `PlanningError` with a `brew install beads` hint on failure
      (Beads ships as a Go binary via Homebrew; it is not a PyPI
      package).
- [x] Body-writing mechanism chosen per `bd create --help` (Beads
...
- [x] `create_task(*, title, description, bd_type, priority, feature,
      ...
      pipeline (Task 3).
- [x] For each `blocker_id`, follow the create with
...
- [x] Parse the returned `bd` task id from stdout (`--silent` makes
      create emit only the id).
- [x] `close_task(task_id)` runs `bd close <id>`, raises on non-zero.
- [x] `list_feature_tasks(feature)` runs
      `bd list --label feature:<feature> --json --limit 0` and returns
      a tuple of `BeadsTaskRef(id, title, labels)` records for open
      tasks.
- [x] All non-zero exits raise `PlanningError` with `bd` stderr
      preserved verbatim (falling back to stdout if stderr is empty).
- [x] Unit tests in `tests/test_transcription_beads.py` using
...
- [x] `src/turma/transcription/__init__.py` exposes
...
- [x] Gate on the `APPROVED` terminal marker in the change directory
...
- [x] Preflight `TRANSCRIBED.md`: refuse without `--force` if present.
- [x] Preflight `list_feature_tasks(feature)` when `TRANSCRIBED.md` is
...
- [x] Implement the pipeline: parse, iterate sections in order,
      ...
      of later sections.
- [x] On full success, write `TRANSCRIBED.md` with feature name,
      timestamp, and the created task IDs in section order.
- [x] `--force` teardown: if `TRANSCRIBED.md` is present, close the
...
- [x] Partial-failure behavior matches the design: no automated
      rollback, clear error surfacing `bd` stderr and orphan IDs.
- [x] Integration tests in `tests/test_transcription_pipeline.py`
      using a stub `BeadsAdapter` and an approved fixture change dir.

### 4. Wire the CLI subcommand

- [x] Add `plan-to-beads` subparser to `src/turma/cli.py` with
      `--feature` (required) and `--force` (flag).
- [x] Construct a `BeadsAdapter()` and dispatch to
...
- [x] Map `PlanningError` to a non-zero exit and print the message.
- [x] `tests/test_transcription_cli.py`: invocations, exit codes,
      stray-flag rejection, `--force` combinations.

### 5. Docs and config surface

- [x] Update `README.md`: new "Plan-to-Beads" section documenting
      the command, the `bd` runtime prerequisite, the `--force`
      semantics, and the partial-failure manual recovery.
- [x] Update `docs/architecture.md` "Task Translation" section if it
      drifts from the committed v1 contract here.
- [x] `.gitignore`: ensure any local `bd` / Dolt state outside the
      repo root is covered if needed (usually not — `.beads/` is the
      repo-local state and is already conventional).
- [x] Note in `turma.example.toml` if any new config fields are
      introduced. v1 does NOT add any; transcription has no tunable
      knobs other than `--force`.

### 6. End-to-end validation

- [x] Smoke script or documented manual run on a small real feature:
...
- [x] Verify `--force` teardown path end-to-end on the same fixture.
- [x] Record the result in a CHANGELOG entry or PR body; no new
